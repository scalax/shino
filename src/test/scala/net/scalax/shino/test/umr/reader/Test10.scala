...
class Test10 extends FlatSpec with Matchers with EitherValues with ScalaFutures with BeforeAndAfterAll with BeforeAndAfter with CustomRep {
  ...
  val db = Database.forURL(s"jdbc:h2:mem:reader_test10;DB_CLOSE_DELAY=-1", driver = "org.h2.Driver", keepAliveConnection = true)
