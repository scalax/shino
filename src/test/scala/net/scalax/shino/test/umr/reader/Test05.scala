...
class Test05 extends FlatSpec with Matchers with EitherValues with ScalaFutures with BeforeAndAfterAll with BeforeAndAfter {
  ...
  val db = Database.forURL(s"jdbc:h2:mem:reader_test05;DB_CLOSE_DELAY=-1", driver = "org.h2.Driver", keepAliveConnection = true)
  ...
  "shape" should "auto map with table and case class" in {
    ...
    val result = await(db.run(friendTq.sortBy(_.id).map(_.getter).to[List].result))
