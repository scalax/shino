...
class Test01 extends FlatSpec with Matchers with EitherValues with ScalaFutures with BeforeAndAfterAll with BeforeAndAfter {
  ...
  case class FriendSort(
    ...
    , age: NullsOrdering = SortBy.default
  )
  object FriendSort {
  ...
  val db = Database.forURL(s"jdbc:h2:mem:sortby_test01;DB_CLOSE_DELAY=-1", driver = "org.h2.Driver", keepAliveConnection = true)
  ...
  "shape" should "auto map with table and case class" in {
    ...
    val result = await(db.run(friendTq.sortBy { s =>
      val i    = s.orderDef
      val sOpt = SortBy.strictMutiplySort(i.strictSort("nick", SortBy.ASC, SortBy.NULLS_LAST))
      sOpt match {
        case Right(s) => s
    ...
    result.toList.map(s => s.copy(id = -1)) should be(List(friend1, friend2, friend3).sortBy(_.nick))
  }
